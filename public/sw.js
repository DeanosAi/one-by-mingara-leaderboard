...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(fetch(event.request).then((response) => {
    ...
    return response;
  }).catch(() => caches.match(event.request).then((cached) => cached || caches.match(scopeUrl.href))));
});
